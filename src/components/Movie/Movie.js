...
class Movie extends Component {
  ...
  componentDidMount() {
    ...
    let moviesEndPoint = `${BASE_URL}/movie/${match.params.movieId}?api_key=${API_KEY}&language=en`;
  ...
  getDirectorsAndActors = (creditsEndPoint) => {
    fetch(creditsEndPoint)
      ...
      .then((credits) => {
        // console.log(credits)
        const filterDirector = credits.crew.filter(
          (person) => person.job === "Director"
        ); // filter directors from all employers
  ...
  render() {
    ...
    return (
      <React.Fragment>
        {" "}
        {loadingActors || loadingMovies || loadingVideos ? (
          <Spinner />
